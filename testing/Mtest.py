...
def _configure(str):
    ...
    config = [
        ('{source}', '/home/xzhang3/MonetDB'),
        ('${build}', '/home/xzhang3/MonetDB'),
...
try:
    ...
    try:
        ...
        p = _configure(os.path.join('NONE', 'lib/python2.7/site-packages'))
        sys.path.insert(0, p)
        import MonetDBtesting.Mfilter as Mfilter
...
""" % {'prog': THISFILE,
       'readme': os.path.join('/home/xzhang3/MonetDB','testing','README'),
       })
    raise
...
def PerformDir(env, testdir, testlist, BusyPorts, all_tests = False) :
    ...
    os.environ['PYTHON2PATH'] += os.path.pathsep + _configure(os.path.join('NONE', 'lib/python2.7/site-packages'))
    os.environ['PYTHON3']   = '/usr/bin/python3'
    if 'PYTHON3PATH' not in os.environ: os.environ['PYTHON3PATH'] = ''
    os.environ['PYTHON3PATH'] += os.path.pathsep + _configure(os.path.join('NONE', 'lib/python3.4/site-packages'))
    if os.name == 'nt':
        os.environ['PYTHON2PATH'] += os.path.pathsep + _configure(os.path.join('NONE', 'lib', 'python2'))
        os.environ['PYTHON3PATH'] += os.path.pathsep + _configure(os.path.join('NONE', 'lib', 'python3'))
...
def DoIt(env, SERVER, CALL, TST, EXT, PRELUDE, TestOutFile, TestErrFile, STIMEOUT, CTIMEOUT, TIMEOUT, ME, MAPIsockets, length, nomito, threads, COND, all_tests) :
    ...
    try:
        if SERVER in ["MAL", "SQL"]:
            ...
            elif (not all_tests or (COND != None and "HAVE_LIBPY" in COND)) and CONDITIONALS['HAVE_LIBPY']:
...
def main(argv) :
    ...
    dft['TSTSRCBASE']     = r"_configure('/home/xzhang3/MonetDB')"
    dft['TSTTRGBASE']     = r"_configure('NONE')"   # or os.getcwd() ?
    if THISFILE == "Mtest.py":
    ...
    cmd_options = [
        ...
        ('TSTSRCBASE', None, 'TSTSRCBASE', '<path>',
         'default: "%s"' % _configure('/home/xzhang3/MonetDB')),
        ('TSTTRGBASE', None, 'TSTTRGBASE', '<path>',
         'default: "%s"' % _configure('NONE')),
        ('quiet', 'q', 'quiet', None,
    ...
    if THISFILE == 'Mtest.py':
        ...
        f = _configure(os.path.join('NONE',dftTSTPREF,'.Mapprove.rc'))
        v = ReadMapproveRc(f)
    ...
    #bp = ""        #_configure(os.path.join('NONE',"bin"))
    #if THISFILE == "Mtest.py":
